...
//	}
	...
	for(var i = 0; i < Features.length; i++) {
		...
		if(fName=="Any") {
			...
			if (value==null||value=="") {
				if(ele.nodeName=="SELECT"){
					var values=ele.value.replace(/\s+/g,"");//去空格
					if(values==''){//可输入时已经有字符了但value值为空
						msg.push("请选择"+name);
					}
				}else{
					msg.push(name+"不能为空");
				...
				msg.push(name+"不能为"+value);
			}
		}else if (fName=="Number") {
			if (value==null||value=="") {continue;}
			if(!isNumber(value)||value<=0){
				msg.push(name+"必须是数字且大于0");
			}
		...
		}else if (fName=="Number3") {
			if (value==null||value=="") {continue;}
			if(!isNumber(value)||value<0){
				msg.push(name+"必须是数字且大于等于0");
			}
		}
           ...
           if(isNaN(fValue)) {
			...
			if(value==""){
				...
				if (!pattern.test(value)) {
					msg.push(name + "身份证格式有误")
				}
			}
		 }else if(fName=="isCharOrNumber") {
			if (value==null||value=="") {continue;}
			...
			if(value==""){
				...
				if (!pattern.test(value)) {
					msg.push(name + "必须为字母和数字的组合。")
				}
			}
         }else if(fName=="isPassword"){
        	 if (value==null||value=="") {continue;}
             if(isNaN(fValue)) {
  				msg.push("校验规则错误");
  			}else{
...
Verify.closeTip = function(ele,evt){
	...
	if(ele._VerifyTip&&ele._VerifyTip!=null&&ele._VerifyTip!='null'){
...
Verify.hasError = function(noCheckArr,ele){
	...
	if(ele){
		ele = $Z(ele);
        arr =  ele.$T("input").concat(ele.$T("textarea")).concat(ele.$T("select"));
	}else{
        arr = $T("input").concat($T("textarea")).concat($T("select"));
	}
	var firstEle = false;
...
window.setInterval(function(){
	var arr = $T("input").concat($T("textarea")).concat($T("select"));
