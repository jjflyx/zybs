...
function docReady(){
	...
	$('[tgg-data="chosen"]').chosen({ no_results_text: "没有查询到", search_contains: true,placeholder_text_single:"请选择" });
}
...
$.fn.extend({
    ...
    updateSelect: function (json_map, content_field, value_field, default_value) {
        if (!this.is("select"))
            return;
        var select = this;
        select.removeClass();
    ...
    formatChosen: function(){
        if (!this.is("select"))
            return;
        var select = this;
        var chosenId = select.attr("id") + "_chzn";
        $("#" + chosenId).remove();
        select.removeClass();
